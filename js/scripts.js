...
(function ($) {
  ...
  var methods = {
    init: function init(options) {
      ...
      global_settings = settings; // Create percentage
      ...
      $(this).append(percentage); // Create text

      var text = $("<div class='progress-text'></div>"); // Custom text

      if (global_settings.text != "percent") {
        text.text(global_settings.text);
      }

      $(this).append(text); // Correct any invalid values
      ...
      appendUnit(global_settings.height); // Apply global_settings

      $(this).css({
        "height": global_settings.height,
        "width": global_settings.width
      });
      $(this).addClass("circular-progress-bar"); // Remove old canvas

      $(this).find("canvas").remove(); // Put canvas inside this

      $(this).append(createCanvas($(this))); // Return allows for chaining

      return this;
    },
    percent: function percent(value) {
      // Change percent
      global_settings.percent = value; // Apply global_settings
      ...
      $(this).children("canvas").remove(); // Put canvas inside this

      $(this).append(createCanvas($(this))); // Return allows for chaining

      return this;
    },
    animate: function animate(value, time) {
      ...
      var num_of_steps = time / 10; // Amount of change each step

      var percent_change = (value - global_settings.percent) / num_of_steps; // Variable conflict, rename this

      var scope = $(this);
      var theInterval = setInterval(function () {
        if (global_settings.percent < value) {
          // Remove old canvas
          scope.children("canvas").remove(); // Increment percent

          global_settings.percent += percent_change; // Put canvas inside this

          scope.append(createCanvas(scope));
        } else {
          clearInterval(theInterval);
        }
      }, 10); // Return allows for chaining
  ...
  function createCanvas(scope) {
    ...
    var canvas_width = removeUnit(global_settings.width.toString()); // Create canvas

    var canvas = document.createElement("canvas");
    canvas.height = canvas_height;
    canvas.width = canvas_width; // Create drawable canvas and apply properties

    var ctx = canvas.getContext("2d");
    ctx.strokeStyle = global_settings.color;
    ctx.lineWidth = global_settings.line_width; // Draw arc

    ctx.beginPath(); // Calculate starting and ending positions

    var starting_radian = calcPos(global_settings.starting_position, -1);
    var ending_radian = calcPos(global_settings.starting_position, global_settings.percent); // Calculate radius and x,y coordinates

    var radius = 0;
    var xcoord = canvas_width / 2;
    var ycoord = canvas_height / 2; // Height or width greater
...
// var btnForm = document.querySelector('.form__btn');
...
//   var token = "1255735512:AAH4YmGBAMMeIVYwGpNlQGiHiTj1KbmKWvo";
//   var chatId = "-392300022";
//   var textMessage = "\u0418\u043C\u044F: ".concat(name, ", \u041F\u043E\u0447\u0442\u0430: ").concat(email, ", \u0422\u0435\u043B\u0435\u0444\u043E\u043D: ").concat(phone);
//   var linkSendTelegramBot = "https://api.telegram.org/bot".concat(token, "/sendMessage?chat_id=").concat(chatId, "&parse_mode=html&text=").concat(textMessage);
//   console.log(textMessage);
//   form.setAttribute('action', linkSendTelegramBot);
//   e.preventDefault();
//   form.requestSubmit();
//   document.location='http://4lexm4rtin.byethost14.com/';
//   return false;
// }
// $(document).ready(function () {
